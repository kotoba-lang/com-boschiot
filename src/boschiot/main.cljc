(ns boschiot.main
  "Kotodama WASM entrypoint for the Boschiot clean-room actor (L4) — Clojure port.
  ...
  facts (`boschiot.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "boschiot")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"boschiot.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "boschiot-compat" :tier tier :entities entities} 200])
...
(defn start! [] :boschiot-compat/ready)
